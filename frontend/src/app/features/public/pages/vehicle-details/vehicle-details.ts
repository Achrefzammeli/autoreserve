import { Component, OnInit, inject } from '@angular/core';
import { ActivatedRoute, Router, RouterLink } from '@angular/router';
import { CommonModule } from '@angular/common';
import { FormBuilder, ReactiveFormsModule, Validators } from '@angular/forms';

import { VehicleService } from '../../../../core/services/vehicle.service';
import { BookingService } from '../../../../core/services/booking.service';
import { AuthService } from '../../../../core/services/auth.service';
import { Loading } from '../../../../shared/components/loading/loading';
import { environment } from '../../../../../environments/environment';

@Component({
  selector: 'app-vehicle-details',
  standalone: true,
  imports: [CommonModule, RouterLink, ReactiveFormsModule, Loading],
  templateUrl: './vehicle-details.html',
  styleUrl: './vehicle-details.css'
})
export class VehicleDetails implements OnInit {
  private fb = inject(FormBuilder);

  vehicle: any;
  loading = true;
  bookingLoading = false;
  bookingError = '';
  bookedDates: any[] = [];

  constructor(
    private route: ActivatedRoute,
    private vehicleService: VehicleService,
    private bookingService: BookingService,
    private auth: AuthService,
    private router: Router
  ) {}

  bookingForm = this.fb.group({
    start_date: ['', Validators.required],
    end_date: ['', Validators.required]
  });

  ngOnInit() {
    const id = Number(this.route.snapshot.paramMap.get('id'));

    this.vehicleService.getVehicleById(id).subscribe({
      next: (data) => { this.vehicle = data; this.loading = false; },
      error: (err) => { console.error(err); this.loading = false; }
    });

    this.vehicleService.getBookedDates(id).subscribe({
      next: data => { this.bookedDates = data; }
    });
  }

  get isLoggedIn(): boolean {
    return this.auth.isLoggedIn();
  }

  get days(): number {
    const start = this.bookingForm.value.start_date;
    const end = this.bookingForm.value.end_date;
    if (!start || !end) return 0;

    const startDate = new Date(start + 'T00:00:00');
    const endDate = new Date(end + 'T00:00:00');
    const diffMs = endDate.getTime() - startDate.getTime();

    return Math.max(Math.ceil(diffMs / (1000 * 60 * 60 * 24)), 1);
  }

  get totalPrice(): number {
    if (!this.vehicle || !this.days) return 0;
    return this.days * this.vehicle.price_per_day;
  }

  imageUrl(): string | null {
    if (!this.vehicle?.image_url) return null;
    return this.vehicle.image_url.startsWith('http')
      ? this.vehicle.image_url
      : `${environment.apiUrl}${this.vehicle.image_url}`;
  }

  statusChip(status: string): string {
    return {
      AVAILABLE: 'chip-success',
      MAINTENANCE: 'chip-warning',
      UNAVAILABLE: 'chip-danger',
    }[status] ?? 'chip-neutral';
  }

  statusLabel(status: string): string {
    return {
      AVAILABLE: 'Disponible',
      MAINTENANCE: 'En maintenance',
      UNAVAILABLE: 'Indisponible',
    }[status] ?? status;
  }

  bookVehicle(): void {
    if (!this.isLoggedIn) {
      this.router.navigate(['/auth/login']);
      return;
    }

    if (this.bookingForm.invalid) {
      this.bookingForm.markAllAsTouched();
      return;
    }

    this.bookingLoading = true;
    this.bookingError = '';

    this.bookingService.createBooking({
      vehicle_id: this.vehicle.id,
      start_date: this.bookingForm.value.start_date,
      end_date: this.bookingForm.value.end_date
    }).subscribe({
      next: () => {
        this.router.navigate(['/client/bookings']);
      },
      error: err => {
        this.bookingLoading = false;
        this.bookingError = err.error?.detail ?? 'La réservation a échoué. Veuillez réessayer.';
      }
    });
  }
}
